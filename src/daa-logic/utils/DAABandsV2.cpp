...
protected:
   ...
   */
  int checkM3 (BandsRegion::Region currentRegion) const {
...
public:
  DAAMonitorsV2 (Daidalus* daa = NULL) : daa(daa) { }
...
public:
  ...
  std::string jsonBands (
    ...
    for (int i = 0; i < daa.horizontalDirectionBandsLength(); i++) {
      ...
      trk += ", \"alert\": \"" + BandsRegion::to_string(daa.horizontalDirectionRegionAt(i));
      trk += "\" }";
    ...
    for (int i = 0; i < daa.horizontalSpeedBandsLength(); i++) {
      ...
      gs += ", \"units\": \"" + hs_units + "\"";
      gs += ", \"alert\": \"" + BandsRegion::to_string(daa.horizontalSpeedRegionAt(i));
      gs += "\" }";
    ...
    for (int i = 0; i < daa.verticalSpeedBandsLength(); i++) {
      ...
      vs += ", \"units\": \"" + vs_units + "\"";
      vs += ", \"alert\": \"" + BandsRegion::to_string(daa.verticalSpeedRegionAt(i));
      vs += "\" }";
    ...
    for (int i = 0; i < daa.altitudeBandsLength(); i++) {
      ...
      alt += ", \"units\": \"" + alt_units + "\"";
      alt += ", \"alert\": \"" + BandsRegion::to_string(daa.altitudeRegionAt(i));
      alt += "\" }";
    ...
    BandsRegion::Region resTrkRegion = daa.regionOfHorizontalDirection(resTrkInternal);
    BandsRegion::Region resTrkRegion_sec = daa.regionOfHorizontalDirection(resTrkInternal_sec);
    ...
    BandsRegion::Region resGsRegion = daa.regionOfHorizontalSpeed(resGsInternal);
    BandsRegion::Region resGsRegion_sec = daa.regionOfHorizontalSpeed(resGsInternal_sec);
    ...
    BandsRegion::Region resVsRegion = daa.regionOfVerticalSpeed(resVsInternal);
    BandsRegion::Region resVsRegion_sec = daa.regionOfVerticalSpeed(resVsInternal_sec);
    ...
    BandsRegion::Region resAltRegion = daa.regionOfAltitude(resAltInternal);
    BandsRegion::Region resAltRegion_sec = daa.regionOfAltitude(resAltInternal_sec);
  ...
  void walkFile () {
    ...
    *printWriter << "\"Wind\": { \"deg\": \"" << Units::to("deg", daa.getWindVelocityFrom().compassAngle()) 

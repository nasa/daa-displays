...
public:
	...
	void printHelpMsg() {
		std::cout << "Version: DAIDALUS++ v" + KinematicBandsParameters::VERSION << std::endl;
	...
	std::string jsonHeader () const {
		const std::string ans = std::string("{ \"language\": \"C++\", \"version\": ") + std::string("\"") + KinematicBandsParameters::VERSION
	...
	void parseCliArgs (char* const args[], int length) {
		...
		for (int a = 1; a < length; a++) {
			if (std::strcmp(args[a], "--help") == 0 || std::strcmp(args[a], "-help") == 0 || std::strcmp(args[a], "-h") == 0) {
				...
				std::cout << KinematicBandsParameters::VERSION << std::endl;
				exit(0);
